...
describe( 'Timer - RT testing', () => {
    ...
    //         globalTimeoutSpy = getTimeoutSpy();
    ...
    //             // @debug
    //             console.log( 'init dispatch test with visibility state = ', visibilityState );
    ...
    //     });
    //     test( 'begins a new cycle if device "sleep" began in between cycles', done => {
    //         const timer = new Timer( noop, MAX_SET_TIMEOUT_DELAY * 3 );
    //         timer.addEventListener( 'cycleEnding', () => { // at the end of first cycle
    //             initDispatchTest( 'hidden' ); // trigger the sleep mode
    ...
    //                 //     })
    //                 // );
    //                 expect( timerSetTimeoutSpy ).not.toHaveBeenCalled();
    //                 resolve( true );
    //             }).finally(() => {
    ...
    //         jest.runAllTimers(); // use up first cycle.
    //     } );
    //     // test( 'resumes timer if device "sleep" interrupted an ongoing cycle', () => {
    //     // } );
    // } );
} );